...
def employee_take_homes_salary(employee_name,employee_gross_income,federal_tax):
    ...
    #1.455 of gross income is medicare tax
    ...
    print '%s take home salary is : %s'%(employee_name,net_income)

    return net_income

def read_employee_salary_csv_file():
    "This functions reads the CSV file and get the gross income from the CSV file"
    # To read the CSV file we have to join the path where it's located 
    taxable_deduction = 12000
    #matched_salary_person = []   
    filename = os.path.abspath(os.path.join('..','training',"employee_payroll.csv"))
    ...
    with open(filename,"r") as fp:
        ...
        for row in csv_file:            
            ...
            #Fetche the employee gross income from the file          
...
if __name__ == "__main__":    
    employee_take_home_salary = read_employee_salary_csv_file()
    print 'The employee take home salary is %s'%employee_take_home_salary
